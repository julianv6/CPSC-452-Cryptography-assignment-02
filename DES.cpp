...
unsigned char* DES::decrypt(const unsigned char* cipherText)
{
	...
	//5. Perform des_encrypt1 in order to decrypt the block using this->key (see sample codes for details)
	des_encrypt1(block, key, DEC);

	unsigned char* decryptedText = new unsigned char[8];
	memset(decryptedText, 0, 8);

	//6. Convert the first long to 4 characters using ltoc()
	ltoc(block[0], decryptedText);
	//7. Convert the second long to 4 characters using ltoc()
